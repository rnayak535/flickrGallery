                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                    ...
                                    <div id="categoryDiv">
                                        <nav class="mt-2">
                                            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview"
                                                ...
                                                <li class="nav-item">
                                                    <a href="javascript:void(0);" class="nav-link makeActiveInactive"
                                                        onclick="getImagesByCategory('{{ $singleCategory['name'] }}');makeActiveInactiveCategory(this);">
                                ...
                                <div class="col-lg-9 col-md-9 col-sm-8 border">
                                    <div class="text-center my-2">
                                        <h3 id="categoryTitle">Images</h3>
                                    </div>
                                    <div id="flickerImagesDiv" class="overflow-auto">
                                        <div class="alert alert-info text-center mt-4">
                                            <h5>Select category from left to see images here.</h5>
                                        </div>
                                    </div>
                                    <div id="singleImageView" class="my-2 d-none">
                                        <div class="row">
                                            <div class="col-md-7">
                                                <img id="setImageSrc" width="100%" height="500px" class="rounded"
                                                    src="">
                                            </div>
                                            <div class="col-md-5">
                                                <div class="my-2 mt-3">
                                                    <span class="h3">Image Details</span>
                                                    <button class="btn btn-primary float-right" onclick="goBack();"><i
                                                            class="fa fa-angle-left" aria-hidden="true"></i> Go
                                                        Back</button>
                                                </div>
...
function makeActiveInactiveCategory(ele) {
    $('.makeActiveInactive').removeClass('active');
    $(ele).addClass('active');
}
...
function getImagesByCategory(categoryName) {
    ...
    $.ajax({
        ...
        beforeSend: function() {
            $("#flickerImagesDiv").html(
                '<div class="text-center mt-4"><i class="fa fa-spinner fa-spin fa-3x fa-fw" aria-hidden="true"></i></div>'
                );
        },
        success: function(result, status, xhr) {
            ...
            $.each(result.items, function(i, item) {
                ...
                imageString += '<a href="javascript:void(0);" class="" onclick="showImage(\'' +
                    categoryName + '\', \'' + item.media.m + '\', \'' + item.tags + '\');">';
                imageString += '<img width="100%" height="200px" class="rounded" src="' + item.media
                    .m + '"></a></div>';
            });
            imageString += '</div>';
        ...
        error: function(xhr, status, error) {
            // console.log(xhr)
            $("#flickerImagesDiv").html("Result: " + status + " " + error + " " + xhr.status + " " + xhr
                .statusText)
        }
    });
}
